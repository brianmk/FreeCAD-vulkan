...
Opens a view in raster mode (VulkanRenderMode=1), verifies the RTX
...
def step():
    ...
    k = steps[0]
    if k == 1:
        # Start from a raster-only view: close the startup document, force the
        # raster render mode, then reopen so the new view is created in raster
        # (the reported "reopen" scenario).
        ...
        log("phase=raster-open (VulkanRenderMode=1)")
    elif k == 3:
