...
FreeCAD. Generated by scanning the fork branch and its bundled Coin submodule.
...
| Current branch | `feat/hdr-luminance-2` |
...
| Fork-only commits (incl. merges) | **403** (299 non-merge, 104 merge) |
| Fork-only diff (vs merge-base) | **478 files, +57,628 / −14,449** |
| Author of all non-merge commits | `phantomcake <brianmgs@pm.me>` |
| Bundled Coin submodule | `brianmk/coin` branch `freecad-master`, **279 fork-only commits** |
...
- **Denoisers**: Intel OIDN, NVIDIA RTX, AMD FSR/DNSR, plus None.
...
- **`tools/fcprobe/`** — unified in-GUI test harness `freecad_probe.py`
  (Session: move/click/key/menu/snapshot/expect/soak), 75+ Vulkan/RT probes,
  suites (`vk_suite.json`, `vk_phase2_suite.json`), pre-flight validation,
  run/report/matrix/compare/soak CLI.
- **`tools/fcprobe/mcp/`** — FreeCAD MCP server + guest (live model building,
  selection, cursor, screenshots, log access).
- **`tools/assistant-addon/`** — in-app Assistant chat panel with MCP control.
...
- **Uncommitted** on `feat/hdr-luminance-2` (not in the 403 commits):
  ...
  (`priv->presentMode`) and `VulkanViewportAdapter`; plus untracked
  `FreeCAD_base/`, `README2.md`, `PERF_COMPARISON.md`.
...
git log --no-merges --oneline 3471328fe9..HEAD
git diff --stat 3471328fe9..HEAD
