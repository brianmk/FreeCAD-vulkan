...
__title__ = "FreeCAD FEM flow velocity base task panel"
__author__ = "Markus Hovorka, Bernd Hahnebach, Uwe Stöhr"
__url__ = "https://www.freecad.org"

## @package base_femflowvelocitytaskpanel
#  \ingroup FEM
#  \brief base task panel for the flow velocity constraints
...
class BaseFlowVelocityTaskPanel(base_femtaskpanel._BaseTaskPanel):
    """Base task panel for the flow velocity constraints.

    Subclasses set ``_uiFile`` and, optionally, ``_selectionTypes``.
    """

    _uiFile = None
    _selectionTypes = ["Solid", "Face"]

    def __init__(self, obj):
        super().__init__(obj)

        self._paramWidget = FreeCADGui.PySideUic.loadUi(
            FreeCAD.getHomePath() + "Mod/Fem/Resources/ui/" + self._uiFile
        )

        # geometry selection widget
        # start with Solid in list!
        self._selectionWidget = selection_widgets.GeometryElementsSelection(
            obj.References, self._selectionTypes, True, False
        )
